...
def collect_articles(context, site, skip_title_contains=None, pages=1, debug=False):
    """사이트 종류에 맞춰 글 목록을 수집한다(최대 pages 페이지).

    네이버: 글 목록 API 우선(공지/필독 자동 제외), 실패 시 DOM 폴백.
    그 외(다음 등): DOM 링크 추출 후 제목 기반 공지/필독/안내 제외.
    """
    if site.type == "naver":
        arts = fetch_naver_articles(
            context, site.url, site.name, skip_title_contains, pages=pages, debug=debug
        )
        if arts:
            return arts
        if debug:
    ...
    arts = scrape_site(context, site.name, site.url, site.link_pattern,
                       pages=pages, page_param=site.page_param, debug=debug)
